...
public class OrderService {
    ...
    public OrderSaveResponseDto save(Long userId, String userRole, OrderSaveRequestDto request) {
        ...
        if(storeService.isStoreClosed(request.getStoreId())) {
        ...
        Order order = Order.toEntity(request.getStoreId(), request.getMenuName(), request.getPrice(), request.getStatus());
        Order savedOrder = orderRepository.save(savedOrder);
